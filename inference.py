...
def parse_args():
    ...
    parser.add_argument("--device1", type=str, required=False, help="device", default="cpu")
    parser.add_argument("--device2", type=str, required=False, help="device", default="cpu")
...
def inference_sentiment(tokenizer, model, text):
            ...
            with open(args.sentiment_filename, "w", encoding="utf-8") as file_write:
                with open(args.reviews_filename, encoding="utf-8") as file_read:
                    for line in file_read:
                        line = line[:-1]
                        idx, text = line.split("\t")
                        pred = inference_sentiment(tokenizer, model, text)
                        for cat in pred:
